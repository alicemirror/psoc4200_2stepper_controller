...
volatile uint8 Comm_curStatus; /* Status byte */
uint8 Comm_fsmState;           /* FSM state   */

/* Variables intended to be used with Buffer 1: Primary slave address */
volatile uint8 * Comm_dataBuffer1; /* Pointer to data buffer 1 */
uint16 Comm_bufSizeBuf1;           /* Size of buffer 1 in bytes      */
uint16 Comm_protectBuf1;           /* Start index of write protected area buffer 1 */
uint16 Comm_offsetBuf1; /* Current offset within buffer 1 */
uint16 Comm_indexBuf1;             /* Current index within buffer 1  */

#if(Comm_SECONDARY_ADDRESS_ENABLE_CONST)
    uint8 Comm_addrBuf1; /* Primary slave address. Used for software comparison   */
    uint8 Comm_addrBuf2; /* Secondary slave address. Used for software comparison */

    /* Variables intended to be used with Buffer 2: Primary slave address */
    volatile uint8 * Comm_dataBuffer2; /* Pointer to data buffer 2 */
    uint16 Comm_bufSizeBuf2;           /* Size of buffer 2 in bytes */
    uint16 Comm_protectBuf2;           /* Start index of write protected area buffer 2 */
    uint16 Comm_offsetBuf2; /* Current offset within buffer 2 */
    uint16 Comm_indexBuf2;             /* Current index within buffer 2 */
...
***************************************/
...
#if(Comm_SECONDARY_ADDRESS_ENABLE_CONST)
    static uint32 Comm_EzI2CUpdateRxMatchReg(uint32 addr1, uint32 addr2);
#endif /* (Comm_SECONDARY_ADDRESS_ENABLE_CONST) */
    ...
    ***************************************/
    ...
    const Comm_EZI2C_INIT_STRUCT Comm_configEzI2C =
    {
        Comm_EZI2C_CLOCK_STRETCHING,
        Comm_EZI2C_MEDIAN_FILTER_ENABLE,
        Comm_EZI2C_NUMBER_OF_ADDRESSES,
        Comm_EZI2C_PRIMARY_SLAVE_ADDRESS,
        Comm_EZI2C_SECONDARY_SLAVE_ADDRESS,
        Comm_EZI2C_SUB_ADDRESS_SIZE,
        Comm_EZI2C_WAKE_ENABLE,
        Comm_EZI2C_BYTE_MODE_ENABLE
    };

    /*******************************************************************************
    * Function Name: Comm_EzI2CInit
    ****************************************************************************//**
    *
    *  Configures the Comm for EZI2C operation.
    *
    ...
    *  After initializing the Comm in EZI2C mode using this function, 
    ...
    *******************************************************************************/
    void Comm_EzI2CInit(const Comm_EZI2C_INIT_STRUCT *config)
    {
        ...
        {
            /* Configure pins */
            Comm_SetPins(Comm_SCB_MODE_EZI2C, Comm_DUMMY_PARAM,
                                                                      Comm_DUMMY_PARAM);

            /* Store internal configuration */
            Comm_scbMode       = (uint8) Comm_SCB_MODE_EZI2C;
            Comm_scbEnableWake = (uint8) config->enableWake;
            Comm_scbEnableIntr = (uint8) Comm_SCB_IRQ_INTERNAL;

            Comm_numberOfAddr  = (uint8) config->numberOfAddresses;
            Comm_subAddrSize   = (uint8) config->subAddrSize;
            ...
            Comm_CTRL_REG     = Comm_GET_CTRL_BYTE_MODE  (config->enableByteMode)    |
                                            Comm_GET_CTRL_ADDR_ACCEPT(config->numberOfAddresses) |
                                            Comm_GET_CTRL_EC_AM_MODE (config->enableWake);

            Comm_I2C_CTRL_REG = Comm_EZI2C_CTRL;

            /* Configure RX direction */
            Comm_RX_CTRL_REG = Comm_EZI2C_RX_CTRL |
                                        Comm_GET_RX_CTRL_MEDIAN(Comm_DIGITAL_FILTER_DISABLE);
                                                ;
            Comm_RX_FIFO_CTRL_REG = Comm_CLEAR_REG;

            /* Set default address and mask */
            if(Comm_EZI2C_PRIMARY_ADDRESS == config->numberOfAddresses)
            {
                Comm_RX_MATCH_REG = Comm_EzI2CUpdateRxMatchReg(config->primarySlaveAddr,
            ...
            Comm_TX_CTRL_REG      = Comm_EZI2C_TX_CTRL;
            Comm_TX_FIFO_CTRL_REG = ((0u != (config->enableClockStretch)) ?
                                                 Comm_CLEAR_REG : Comm_EZI2C_TX_FIFO_CTRL);
           ...
           Comm_INTR_SPI_EC_MASK_REG = Comm_NO_INTR_SOURCES;
            ...
            Comm_INTR_SLAVE_MASK_REG  = Comm_EZI2C_INTR_SLAVE_MASK;
            ...
            (void) CyIntSetVector(Comm_ISR_NUMBER, (0u != (config->enableClockStretch)) ?
                                                                      (&Comm_EZI2C_STRETCH_ISR) :
                                                                      (&Comm_EZI2C_NO_STRETCH_ISR));

            if(0u != (config->enableClockStretch))
            {
                /* Configure interrupt sources */
                Comm_INTR_SLAVE_MASK_REG |= Comm_INTR_SLAVE_I2C_ADDR_MATCH;
                Comm_INTR_RX_MASK_REG     = Comm_NO_INTR_SOURCES;
            }
            else
            {
                /* Enable Auto ACK/NACK features */
                Comm_I2C_CTRL_REG |= Comm_EZI2C_CTRL_AUTO;

                /* Configure RX interrupt source */
                Comm_INTR_SLAVE_MASK_REG |= Comm_INTR_SLAVE_I2C_START;
            ...
            Comm_addrBuf1 = (uint8) config->primarySlaveAddr;
            Comm_addrBuf2 = (uint8) config->secondarySlaveAddr;
        }
    }
    ...
    *******************************************************************************/
    ...
    {
        /* Configure I2C interface */
        Comm_CTRL_REG     = Comm_EZI2C_DEFAULT_CTRL;
        Comm_I2C_CTRL_REG = Comm_EZI2C_DEFAULT_I2C_CTRL;
        ...
        /* Configure RX direction */
        Comm_RX_CTRL_REG      = Comm_EZI2C_DEFAULT_RX_CTRL;
        Comm_RX_FIFO_CTRL_REG = Comm_EZI2C_DEFAULT_RX_FIFO_CTRL;

        /* Set default address and mask */
        Comm_RX_MATCH_REG     = Comm_EZI2C_DEFAULT_RX_MATCH;

        /* Configure TX direction */
        Comm_TX_CTRL_REG      = Comm_EZI2C_DEFAULT_TX_CTRL;
        Comm_TX_FIFO_CTRL_REG = Comm_EZI2C_DEFAULT_TX_FIFO_CTRL;
        ...
        (void) CyIntSetVector(Comm_ISR_NUMBER, &Comm_EZI2C_STRETCH_ISR);
    #else
        (void) CyIntSetVector(Comm_ISR_NUMBER, &Comm_EZI2C_NO_STRETCH_ISR);
        ...
        /* Configure interrupt sources */
    #if (!Comm_CY_SCBIP_V1)
        Comm_INTR_SPI_EC_MASK_REG = Comm_EZI2C_DEFAULT_INTR_SPI_EC_MASK;
    #endif /* (!Comm_CY_SCBIP_V1) */

        Comm_INTR_I2C_EC_MASK_REG = Comm_EZI2C_DEFAULT_INTR_I2C_EC_MASK;
        Comm_INTR_SLAVE_MASK_REG  = Comm_EZI2C_DEFAULT_INTR_SLAVE_MASK;
        Comm_INTR_MASTER_MASK_REG = Comm_EZI2C_DEFAULT_INTR_MASTER_MASK;
        Comm_INTR_RX_MASK_REG     = Comm_EZI2C_DEFAULT_INTR_RX_MASK;
        Comm_INTR_TX_MASK_REG     = Comm_EZI2C_DEFAULT_INTR_TX_MASK;
        ...
        Comm_addrBuf1 = Comm_EZI2C_PRIMARY_SLAVE_ADDRESS;
        Comm_addrBuf2 = Comm_EZI2C_SECONDARY_SLAVE_ADDRESS;
    #endif /* (Comm_SECONDARY_ADDRESS_ENABLE_CONST) */
    }
...
/*******************************************************************************
* Function Name: Comm_EzI2CGetActivity
****************************************************************************//**
*
...
*  - Comm_EZI2C_STATUS_READ1 - Read transfer complete. The transfer 
...
*  - Comm_EZI2C_STATUS_WRITE1 - Write transfer complete. The buffer 
...
*  - Comm_EZI2C_STATUS_READ2 - Read transfer complete. The transfer
*    used the secondary slave address. The error condition status bit must be 
*    checked to ensure that read transfer was completed successfully.
*  - Comm_EZI2C_STATUS_WRITE2 - Write transfer complete. The buffer
*    content was modified. The transfer used the secondary slave address. 
*    The error condition status bit must be checked to ensure that write 
*    transfer was completed successfully.
*  - Comm_EZI2C_STATUS_BUSY - A transfer intended for the primary 
...
*  - Comm_EZI2C_STATUS_ERR - An error occurred during a transfer 
...
*    Comm_EZI2C_STATUS_ERR and Comm_EZI2C_STATUS_WRITE1/2 
*    is set. It is recommended to discard buffer content in this case.
*
* \globalvars
*  Comm_curStatus - used to store the current status of the EZI2C 
*  slave.
*
*******************************************************************************/
uint32 Comm_EzI2CGetActivity(void)
{
    ...
    status = Comm_curStatus;

    /* Relay on address match event from HW as bus busy status */
    if(Comm_EZI2C_SCL_STRETCH_DISABLE)
    {
        /* For Comm_CY_SCBIP_V0 the wake is prohibited by customizer */
        if(Comm_EZI2C_EC_AM_ENABLE)
        {
            status |= Comm_CHECK_INTR_I2C_EC(Comm_INTR_I2C_EC_WAKE_UP) ?
                        Comm_EZI2C_STATUS_BUSY : 0u;
        }
        else
        {
            status |= Comm_CHECK_INTR_SLAVE(Comm_INTR_SLAVE_I2C_ADDR_MATCH) ?
                        Comm_EZI2C_STATUS_BUSY : 0u;
        }
    ...
    Comm_curStatus &= ((uint8) ~Comm_EZI2C_CLEAR_STATUS);
...
/*******************************************************************************
* Function Name: Comm_EzI2CSetAddress1
****************************************************************************//**
*
...
*
* \globalvars
*  Comm_addrBuf1 - used to store the primary 7-bit slave address 
*  value.
*
*******************************************************************************/
void Comm_EzI2CSetAddress1(uint32 address)
{
    ...
    {
        Comm_addrBuf1 = (uint8) address;

        Comm_RX_MATCH_REG = 
                        Comm_EzI2CUpdateRxMatchReg(address, (uint32) Comm_addrBuf2);
    }
    else
    {
        uint32 matchReg;
        ...
        /* Set address. */
        matchReg &= (uint32) ~Comm_RX_MATCH_ADDR_MASK;
...
/*******************************************************************************
* Function Name: Comm_EzI2CGetAddress1
****************************************************************************//**
*
...
uint32 Comm_EzI2CGetAddress1(void)
{
    ...
    if(Comm_SECONDARY_ADDRESS_ENABLE)
    {
        address = (uint32) Comm_addrBuf1;
    }
    else
    {
        address = (Comm_GET_RX_MATCH_ADDR(Comm_RX_MATCH_REG) >>
...
/*******************************************************************************
* Function Name: Comm_EzI2CSetBuffer1
****************************************************************************//**
*
...
*
* \globalvars
*  Comm_dataBuffer1 – the pointer to the buffer to be exposed to the
*  master on a primary address.
*  Comm_bufSizeBuf1 - the size of the buffer to be exposed to the 
*  master on a primary address.
*  Comm_protectBuf1 - the start index of the read-only region in the
...
void Comm_EzI2CSetBuffer1(uint32 bufSize, uint32 rwBoundary, volatile uint8 * buffer)
{
    if (NULL != buffer)
    {
        ...
        Comm_dataBuffer1 =  buffer;
        Comm_bufSizeBuf1 = (uint16) bufSize;
        Comm_protectBuf1 = (uint16) rwBoundary;
...
/*******************************************************************************
* Function Name: Comm_EzI2CSetReadBoundaryBuffer1
****************************************************************************//**
*
...
void Comm_EzI2CSetReadBoundaryBuffer1(uint32 rwBoundary)
{
...
#if(Comm_SECONDARY_ADDRESS_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: Comm_EzI2CUpdateRxMatchReg
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    static uint32 Comm_EzI2CUpdateRxMatchReg(uint32 addr1, uint32 addr2)
    {
        ...
        matchReg  = (uint32) (Comm_GET_I2C_8BIT_ADDRESS(matchReg) << Comm_RX_MATCH_MASK_POS);
        matchReg |= Comm_GET_I2C_8BIT_ADDRESS(addr1);

        return(matchReg);
    }

    /*******************************************************************************
    * Function Name: Comm_EzI2CSetAddress2
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  Comm_addrBuf2 - used to store the secondary 7-bit slave address 
    *  value.
    *
    *******************************************************************************/
    void Comm_EzI2CSetAddress2(uint32 address)
    {
        Comm_addrBuf2 = (uint8) address;

        Comm_RX_MATCH_REG = 
                        Comm_EzI2CUpdateRxMatchReg((uint32) Comm_addrBuf1, address);
    }


    /*******************************************************************************
    * Function Name: Comm_EzI2CGetAddress2
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    ...
    {
        return((uint32) Comm_addrBuf2);
    }


    /*******************************************************************************
    * Function Name: Comm_EzI2CSetBuffer2
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  Comm_dataBuffer2 – the pointer to the buffer to be exposed to the
    *  master on a secondary address.
    *  Comm_bufSizeBuf2 - the size of the buffer to be exposed to the 
    *  master on a secondary address.
    *  Comm_protectBuf2 - the start index of the read-only region in the
    ...
    *******************************************************************************/
    void Comm_EzI2CSetBuffer2(uint32 bufSize, uint32 rwBoundary, volatile uint8 * buffer)
    {
        if (NULL != buffer)
        {
            ...
            Comm_dataBuffer2 =  buffer;
            Comm_bufSizeBuf2 = (uint16) bufSize;
            Comm_protectBuf2 = (uint16) rwBoundary;
    ...
    /*******************************************************************************
    * Function Name: Comm_EzI2CSetReadBoundaryBuffer2
    ****************************************************************************//**
    *
    *  Sets the read only boundary in the data buffer to be exposed to the I2C
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: Comm_EzI2CSaveConfig
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void Comm_EzI2CSaveConfig(void)
    {
        ...
        if(Comm_EZI2C_SCL_STRETCH_ENABLE)
        {
        ...
        enableInt = (uint8) Comm_INTR_I2C_EC_WAKE_UP;
        ...
        if(Comm_EZI2C_SCL_STRETCH_ENABLE)
        {
            ...
            Comm_SetI2CExtClkInterruptMode(enableInt);
        }
        else
        {
            for(;;) /* Wait for end of transaction intended to slave */
            {
                if(0u == (Comm_GetI2CExtClkInterruptSource() & Comm_INTR_I2C_EC_WAKE_UP))
                {
            ...
            Comm_SetSlaveInterruptMode(Comm_NO_INTR_SOURCES);
    ...
    /*******************************************************************************
    * Function Name: Comm_EzI2CRestoreConfig
    ****************************************************************************//**
    *
    ...
    void Comm_EzI2CRestoreConfig(void)
    {
        ...
        else
        {
            ...
            */
            ...
            Comm_SetI2CExtClkInterruptMode(Comm_NO_INTR_SOURCES);
            Comm_SetSlaveInterruptMode(Comm_EZI2C_INTR_SLAVE_MASK |
