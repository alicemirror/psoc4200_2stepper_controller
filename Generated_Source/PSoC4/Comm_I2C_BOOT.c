...
static uint8 Comm_slReadBuf[Comm_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 Comm_slWriteBuf[Comm_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 Comm_applyBuffer;
...
static void Comm_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: Comm_I2CCyBtldrCommStart
****************************************************************************//**
*
...
*
* \globalvars
*  Comm_applyBuffer - the flag to release the buffer with a response
*  to be read by the host.
*
*******************************************************************************/
void Comm_I2CCyBtldrCommStart(void)
{
    Comm_I2CSlaveInitWriteBuf(Comm_slWriteBuf, Comm_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    Comm_I2CSlaveInitReadBuf (Comm_slReadBuf, 0u);

    Comm_SetCustomInterruptHandler(&Comm_I2CResposeInsert);
    Comm_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: Comm_I2CCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: Comm_I2CCyBtldrCommReset
****************************************************************************//**
*
...
*
* \globalvars
*  Comm_slRdBufSize - used to store slave read buffer size.
*  Comm_slRdBufIndex - used to store the current index within the
*  slave read buffer.
...
/*******************************************************************************
* Function Name: Comm_I2CCyBtldrCommRead
****************************************************************************//**
*
...
* \globalvars
*  Comm_slStatus  - used to store current status of I2C slave.
*  Comm_slWriteBuf - used to store received command.
*  Comm_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus Comm_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (Comm_I2C_SSTAT_WR_CMPLT & Comm_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) Comm_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) Comm_slWriteBuf,
                                              Comm_BYTES_TO_COPY(*count, size));
...
/*******************************************************************************
* Function Name: Comm_I2CCyBtldrCommWrite
****************************************************************************//**
*
...
*
* \globalvars
*  Comm_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
cystatus Comm_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) Comm_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        Comm_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: Comm_I2CResposeInsert
****************************************************************************//**
*
...
static void Comm_I2CResposeInsert(void)
{
    if (Comm_CHECK_INTR_SLAVE_MASKED(Comm_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (Comm_CHECK_I2C_STATUS(Comm_I2C_STATUS_S_READ))
        {
            ...
            if (0u != Comm_applyBuffer)
            {
                Comm_slRdBufSize  = Comm_applyBuffer;
                Comm_slRdBufIndex = 0u;
                Comm_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (Comm_slRdBufIndex != Comm_slRdBufSize)
            {
                Comm_slRdBufIndex = Comm_slRdBufSize;
            }
        }
    }
}
